...
class JitsiVirtualClass extends StatefulWidget {
  final String type;
  const JitsiVirtualClass({Key? key, required this.type}) : super(key: key);
  @override
...
class _JitsiVirtualClassState extends State<JitsiVirtualClass> {
  ...
  void initState() {
    ...
    print(_userController.selectedRecord.value.id);
  }
                  ...
                  ? StudentRecordWidget(
                      onTap: (Record record) {
                        setState(() {
                          ...
                          virtualClass = getAllMeeting(recordId: _userController.selectedRecord.value.id);
                        });
                      },
                    )
  ...
  Future<VirtualClass> getAllMeeting({int? recordId}) async {
    ...
    final _url = widget.type == "class" ? Uri.parse(InfixApi.getVirtualClass(recordId!, 'jitsi')) : Uri.parse(InfixApi.getVirtualMeeting('jitsi'));
    print(_url);
    final response = await http.get(_url, headers: Utils.setHeader(_token.toString()));
...
class JitsiMeetingRow extends StatelessWidget {
              ...
              children: <Widget>[
                ...
                ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    backgroundColor: meeting.status == "join" || meeting.status == "started"
                  ...
                  onPressed: () async {
                   if (meeting.status == "join" ||
                    ...
                    _launchOtherApp('yourUsername', 'yourPassword');
  ...
  void _launchOtherApp(String username, String password) async {
    // Construct the URL with data credentials
    String mettingid='45677267';
    final  urlString   = 'myvcapp://login?username=$username&password=$password&meetingid=$mettingid';
    final url = Uri.parse(urlString);
    Utils.showToast("dsadasdasd");
